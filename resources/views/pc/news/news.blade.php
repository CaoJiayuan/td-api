    ...
    <script>
        function load_more_msg() {
            ...
            if (msg_list.height() + msg_list[0].scrollTop >= msg_list[0].scrollHeight - 30) {
                ...
                $.get('/api/news?page=' + index).done(function (data) {
                    var html = '';
                    for (var i in data) {
                        html += '<li><a><span style="cursor: pointer" data-id=' + data[i].id + '>' + data[i].title + '</span></a></li>'
        ...
        function load_content(id) {
            ...
            var content = $('#content');
            $.get('/api/news/' + id).done(function (data) {
                var descHtml = '<div class="title"><h3 style="font-weight: bolder">' + data.title + '</h3></div><div class="article-desc " style="height: 18px"> <span class="date ">' + '</span> <span >时间：' + new Date(parseInt(data.published_at)*1000).toLocaleDateString().replace(/\//g, "-") + '  ' + new Date(parseInt(data.published_at)*1000).toTimeString().substr(0, 5) + ' </span> <span >分享到 <span class="jiathis_style" style="display: inline-block"><a class="jiathis_button_qzone"></a><a class="jiathis_button_tsina"></a></span></span> </div> <article style="padding:10px 50px">' + data.content + '</article>'+'<script type="text/javascript" src="http://v3.jiathis.com/code_mini/jia.js?uid=" charset="utf-8">';
